...
regression_keys=[x.replace("_cls","_time") for x in regression_keys]
...
class TimeSeries:
    ...
    def select_target(self):
        if st.session_state.setup_time is None:
            st.subheader("Set up your environment")
            st.divider()
            target = st.pills("Select the target column", self.dataset.columns)
            if target:
                st.session_state.setup_time = setup(data=self.dataset, target=target)
    ...
    def compare_models(self):
        if st.session_state.compare_time is None and st.session_state.setup_time is not None:
            st.session_state.compare_time = compare_models()
            st.session_state.compare_summary = pull()
            st.dataframe(st.session_state.compare_summary)
        elif st.session_state.compare_time is not None:
    ...
    def create_model(self):
        self.select_target()
        self.compare_models()
        if st.session_state.create_time is None and st.session_state.setup_time is not None:
            selected_model = st.pills("Choose a model to create", self.models_list)
            if selected_model:
                st.session_state.create_time = create_model(selected_model)
                st.session_state.create_time_df = pull()
                st.dataframe(st.session_state.create_time_df)

                st.session_state.predict_time = predict_model(st.session_state.create_time)
                st.session_state.predict_time_df = pull()
                st.dataframe(st.session_state.predict_time_df)
        elif st.session_state.create_time is not None:
            st.info("Model already created")
            st.dataframe(st.session_state.create_time_df)
            st.dataframe(st.session_state.predict_time_df)

    def ensemble_model(self):
        if st.session_state.ensemble_time is None and st.session_state.create_time is not None:
            st.session_state.ensemble_time = ensemble_model(st.session_state.create_time)
            st.session_state.ensemble_time_df = pull()
            st.dataframe(st.session_state.ensemble_time_df)
            st.session_state.predict_ensemble_time = predict_model(st.session_state.ensemble_time)
            st.session_state.predict_ensemble_time_df = pull()
            st.dataframe(st.session_state.predict_ensemble_time_df)
        elif st.session_state.ensemble_time is not None:
    ...
    def tune_model(self):
        if st.session_state.tune_time is None and st.session_state.create_time is not None:
            st.session_state.tune_time = tune_model(st.session_state.create_time)
            st.session_state.tune_time_df = pull()
            st.dataframe(st.session_state.tune_time_df)
            st.session_state.predict_tune_time = predict_model(st.session_state.tune_time)
            st.session_state.predict_tune_time_df = pull()
            st.dataframe(st.session_state.predict_tune_time_df)
        elif st.session_state.tune_time is not None:
    ...
    def tune_ensemble_model(self):
        if st.session_state.tune_ensembled_time is None:
            st.session_state.tune_ensembled_time = tune_model(st.session_state.ensemble_time)
            st.session_state.tune_ensembled_time_df = pull()
            st.dataframe(st.session_state.tune_ensembled_time_df)
            st.session_state.predict_tune_ensemble_time = predict_model(st.session_state.tune_ensembled_time)
            st.session_state.predict_tune_ensemble_time_df = pull()
            st.dataframe(st.session_state.predict_tune_ensemble_time_df)
        elif st.session_state.tune_ensembled_time is not None:
    ...
    def save_models(self):
        ...
        if choose_model_to_save == "Normal Model":
            if st.session_state.create_time is not None:
                if st.toggle("Provide a custom name for the model"):
                    name_model = st.text_input("Enter model name")
                    if name_model:
                        st.session_state.save_create_time = save_model(st.session_state.create_time, name_model)
                    ...
                    if pn:
                        st.session_state.save_create_time = save_model(st.session_state.create_time, "normal model")
            ...
            if st.session_state.ensemble_time is not None:
                if st.toggle("Provide a custom name for the ensemble model"):
                    name_ensemble = st.text_input("Enter ensemble model name")
                    if name_ensemble:
                        st.session_state.save_ensemble_time = save_model(st.session_state.ensemble_time, name_ensemble)
            ...
            if st.session_state.tune_time is not None:
                if st.toggle("Provide a custom name for the tuned model"):
                    name_tune = st.text_input("Enter tuned model name")
                    if name_tune:
                        st.session_state.save_tune_time = save_model(st.session_state.tune_time, name_tune)
            ...
            if st.session_state.tune_ensembled_time is not None:
                if st.toggle("Provide a custom name for the tuned ensemble model"):
                    ...
                    if name_tune_ensemble:
                        st.session_state.save_ensembled_tune_time = save_model(st.session_state.tune_ensembled_time, name_tune_ensemble)
    ...
    def reCreate(self):
        st.session_state.create_time = None
        st.session_state.create_time_df = None
        st.session_state.predict_time = None
        st.session_state.predict_time_df = None
        st.session_state.ensemble_time = None
        st.session_state.ensemble_time_df = None
        st.session_state.tune_time = None
        st.session_state.tune_time_df = None
